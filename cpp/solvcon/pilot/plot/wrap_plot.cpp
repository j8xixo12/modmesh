    ...
    : public WrapBase<WrapPlotColor, PlotColor>
{
        ...
        : root_base_type(mod, pyname, pydoc)
    {
        ...
        (*this)
            ...
            // py::self, not a lambda: only the operator form carries
            // py::is_operator(), so `color == None` is False, not a TypeError.
            .def(py::self == py::self) // NOLINT(misc-redundant-expression)
            .def(py::self != py::self) // NOLINT(misc-redundant-expression)
            // Defining __eq__ makes pybind11 drop __hash__; put it back.
            .def(
                "__hash__",
