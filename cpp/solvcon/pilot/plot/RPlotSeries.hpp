...
/**
 * One xy series: a contiguous SimpleCollector<double> pair plus its stroke
 * style. set_data() copies its operands, so a later write through the NumPy
 * array behind one does not reach the samples. Spans die at the next
 * set_data(); not thread-safe.
 */
class RPlotSeries
{
public:
